...
case "$PROBE" in
  ...
  3??)
    ...
    #
    #    API ما هرگز `/restaurant/stats` را ریدایرکت نمی‌کند.  این
    #    واقعاً رخ داد: درگاه ۳۲۰۰ را سرورِ Next.js پروژهٔ دیگری روی
    #    همان ماشین گرفته بود و آزمون به آن می‌خورد.
    #
    #    شکست دادنش گمراه‌کننده است — عیبی در محصول نیست — ولی سکوت
    #    هم غلط است: باید گفته شود چرا اجرا نشد.
