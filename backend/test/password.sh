...
code_of() {
  c=$(login_code "$1")
  if [ "$c" = "429" ]; then
    # ⚠️ به stderr، نه stdout.
    #
    # خروجی این تابع داخل `$( )` گرفته می‌شود، پس هر چیزی که روی
    # stdout چاپ شود بخشی از **مقدار** می‌شود.  سنجه با «got= … سقف
    # ورود پر شد ۲۰۰ want=200» می‌افتاد — شبیه اشکال منطقی، ولی فقط
    # پیامِ خودِ ابزار بود که به عدد چسبیده بود.
