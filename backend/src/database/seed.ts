...
const CHART_OF_ACCOUNTS: Array<{
  ...
  // مهاجرت ۰۸۱ برای شرکت‌های موجود؛ نصبِ تازه از این‌جا.
  // جدا از ۱۱۰۴ است چون شمارشِ انبار پیدایش نمی‌کند — یکی‌بودنشان یعنی
  // هر انبارگردانی کسری نشان می‌دهد.
  { code: '1108', name: 'موجودی کالای امانی نزد دیگران', type: 'ASSET', parent: '1100' },
...
async function seed(): Promise<void> {
  ...
  try {
    ...
    // ----- اشتراکِ شرکت نمونه -----
    //
    // ⚠️ مهاجرت ۰۷۱ اشتراک را با `INSERT ... FROM "Company"` پر می‌کند،
    //    که فقط برای نصب‌های **موجود** کار می‌کند: مهاجرت پیش از ساختِ
    //    شرکت اجرا می‌شود، پس روی نصبِ تازه صفر ردیف می‌سازد.
    //
    //    نگهبانِ `accounts` دقیقاً همین را گرفت — و درست گرفت: پایگاهِ
    //    داده‌ی تازه‌ی این پروژه واقعاً هیچ ردیفِ اشتراکی نداشت.
    //
    // ⚠️ چرا ADVANCED و بی‌حد؟  چون همان چیزی است که مهاجرت به شرکت‌های
    //    موجود می‌دهد.  هر چیزِ دیگری یعنی نصبِ تازه و نصبِ ارتقایافته
    //    دو رفتارِ متفاوت دارند — و آن تفاوت روزی سرِ مشتری درمی‌آید.
    await pool.query(
      `INSERT INTO "Subscription" (id, "companyId", plan, status, note)
       VALUES ($1, $2, 'ADVANCED', 'ACTIVE', 'نصبِ نمونه — بی‌حد')
       ON CONFLICT ("companyId") DO NOTHING`,
      ['seed-subscription', SEED_COMPANY],
    );
